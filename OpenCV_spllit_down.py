...
def split_up(path, num):
    ...
    # cv2.imwrite("down_record\\1.png", blur)
    ...
    # cv2.imwrite("down_record\\box.png", img_color2)
    ...
    for row in range(image_height):
        for col in range(image_width):
            if row < top+100 or row > down or col < left + width_set or col > right - width_set:
                img_color1[row][col] = (0, 0, 0)

    # cv2.imwrite("down_record\\box.png", img_color1)
    ...
    if col_up != []:
        result= np.polyfit(col_up, row_up, 4, full=True)
        weights_up = result[0]
        ...
        # cv2.waitKey(0)
        cv2.imwrite("dataset/lower_part_new/{}.png".format(num), img_color1)
...
if __name__ == '__main__':
    ...
    #             img_color1[i][j] = 0
    #
    # cv2.imwrite("down_record\\upper_1.png", img_color1)
